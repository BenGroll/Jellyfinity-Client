...
void main() {
  ...
  test('lists downloaded albums as ordinary album pages, marked cached', () async {
  ...
  test('lists completed track downloads and honours the search term', () async {
    ...
    expect(all.items.map((t) => t.name), containsAll(['So What', 'Blue in Green']));

    final filtered = (await source.tracks(searchTerm: 'blue')).valueOrNull!;
    expect(filtered.items.single.name, 'Blue in Green');
  });

  test('a server-dropped track download reads as "only on this device"', () async {
    store.records[mediaId('t1')] = downloadRecord(
      mediaId('t1'),
      title: 'So What',
      state: DownloadState.completed,
