...
static const UINT mediaButtonMessage = RegisterWindowMessageW(L"Jellyfinity.MediaButton");
...
namespace audio_service_win
{
      ...
      flutter::PluginRegistrarWindows *registrar)
  {
    ...
    messageWindow = GetAncestor(registrar->GetView()->GetNativeWindow(), GA_ROOT);
    registrar->RegisterTopLevelWindowProcDelegate(
        [](HWND, UINT message, WPARAM button, LPARAM) -> std::optional<LRESULT> {
          if (message != mediaButtonMessage) return std::nullopt;
          using Button = winrt::Windows::Media::SystemMediaTransportControlsButton;
          const char* name = nullptr;
          switch (static_cast<Button>(button)) {
            case Button::Play: name = "play"; break;
            case Button::Pause: name = "pause"; break;
            case Button::Next: name = "next"; break;
            case Button::Previous: name = "previous"; break;
            case Button::Stop: name = "stop"; break;
            case Button::FastForward: name = "fastForward"; break;
            case Button::Rewind: name = "rewind"; break;
            default: break;
          }
          if (name && channel) {
            channel->InvokeMethod("onSMTCButtonPressed",
  ...
  static void SetupSMTC()
  {
    if (mediaPlayer == nullptr)
    {
      ...
      smtc.ButtonPressed([](auto const &, winrt::Windows::Media::SystemMediaTransportControlsButtonPressedEventArgs const &args)
                         {
                           // WinRT raises this on a worker thread. Flutter's
                           // messenger must only be used on the platform thread.
                           PostMessageW(messageWindow, mediaButtonMessage,
